...
def _select_requested_plevs(
    ...
    """
    select requested pressure levels while preserving the plev dimension
...
def _prepare_zonal_mean(
    ...
    """
    converts units, then computes zonal + time mean
    returns [plev, lat]
    """
...
def _make_suptitle(plot_cfg, long_name: str, unit: str, start: str, end: str) -> str:
...
def _plot_single_panel_figure(da2d, title, long_name, unit, plot_cfg, start, end, vmin=None, vmax=None, levels=None, ticks=None):
    ...
    fig.suptitle(_make_suptitle(plot_cfg, long_name, unit, start, end), y=0.96)
...
def _plot_panel_row(panels, long_name, unit, plot_cfg, start, end, vmin=None, vmax=None, levels=None, ticks=None):
    n_panels = len(panels)
    ...
    fig.suptitle(_make_suptitle(plot_cfg, long_name, unit, start, end), y=0.96, x=0.43)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
        ...
        for model_name in plot_cfg.models:
            ...
            if plot_cfg.all_single_plots:
                ...
                outfiles = {
                    tag: os.path.join(
                        outdir,
                        f"{var}_{model_tag}_{tag}_{diff_tag}_{start_tag}-{end_tag}.png",
                    )
                ...
                row_outfile = os.path.join(
                    outdir,
                    f"{var}_{model_tag}_{diff_tag}_{start_tag}-{end_tag}.png",
                )
